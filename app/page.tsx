...
export default function Home() {
  ...
  async function generatePaths() {
    ...
    try {
      ...
      const payload = { topic, direction, perspective: { ...lens, caseTitle: selectedCase?.caseTitle || fallback.caseTitle, connection: selectedCase?.connection || selectedCase?.description || fallback.connection, question: selectedCase?.question || fallback.question } };
      let approaches: Path[] | null = null;
      for (let retry = 0; retry < 2 && !approaches; retry += 1) {
        const response = await fetch("/api/inquiry/expand", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
        const data = await response.json();
        if (response.ok && Array.isArray(data.approaches) && data.approaches.length === 3) approaches = data.approaches;
      }
      if (!approaches) throw new Error();
      setPaths(approaches); setPathIndex(0); setStep(3);
    } catch { setPaths(null); setNotice("세 갈래 탐구 내용을 완성하지 못했습니다. 잠시 후 다시 시도해 주세요."); }
    setLoading(false);
  }
  ...
  return <main className="inquiry-page"><header className="page-nav"><button onClick={reset}>← 처음</button><span>학생 탐구</span><strong>탐구 주제 잡기</strong></header>{step > 1 && <div className="context-chips"><span>주제&nbsp; {topic}</span><i>→</i><b>결&nbsp; {direction === "humanities" ? "인문·사회·예술" : "과학·기술·수리"}</b><i>→</i><b>관점&nbsp; {lens.name}</b></div>}<section className={`stage stage-${step}`}>
    ...
    {step === 2 && <div><Header eyebrow="관점 — 10가지 사고 형식" title="이 주제를 어떤 관점으로 파고들래요?" description="입력한 주제와 연구 자료를 바탕으로 관점별 실제 사례를 준비했습니다. 마음이 끌리는 카드 하나를 골라 보세요." />{loading ? <LoadingCards /> : generated?.perspectives?.length || showFallbackCases ? <>{notice && <div className="inline-notice"><span>{notice}</span><button onClick={generatePerspectives}>Gemini로 다시 생성</button></div>}<div className="lens-cards">{lenses.map((item, index) => { const ai = generated?.perspectives?.find((value) => value.key === item.key); const fallback = fallbackCase(topic, item); return <article key={item.key} role="button" tabIndex={0} onClick={() => setLensIndex(index)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setLensIndex(index); }} className={lensIndex === index ? "selected" : ""}><small>관점 · {item.name}</small><h3>{ai?.caseTitle || fallback.caseTitle}</h3><p>{ai?.connection || ai?.description || fallback.connection}</p><em>탐구 질문 · {ai?.question || fallback.question}</em>{ai?.source && <a href={ai.source.url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>연계 자료 · {ai.source.type} · {ai.source.title} ↗</a>}</article>; })}</div><FooterActions back={() => setStep(1)} next={() => setStep(3)} nextLabel="선택 사례 살펴보기 →" /></> : <div className="status-panel"><strong>사례 카드가 아직 준비되지 않았어요.</strong><p>{notice || "생성 결과가 비어 있습니다. 다시 시도하거나 기본 관점으로 계속할 수 있습니다."}</p><div><button onClick={generatePerspectives}>다시 생성</button><button className="next" onClick={() => setShowFallbackCases(true)}>기본 관점으로 계속 →</button></div></div>}</div>}
    {step === 3 && <div><Header eyebrow="선택 사례 — 탐구 출발점" title="고른 사례를 한 번 더 살펴볼까요?" description="이 사례의 대상·조건·질문을 확인한 뒤, 다음 페이지에서 구체적인 세 갈래 탐구 결을 생성합니다." /><div className="case-preview"><small>선택한 관점 · {lens.name}</small><h2>{selectedCase?.caseTitle || fallbackCase(topic, lens).caseTitle}</h2><p>{selectedCase?.connection || selectedCase?.description || fallbackCase(topic, lens).connection}</p><strong>탐구 질문 · {selectedCase?.question || fallbackCase(topic, lens).question}</strong>{selectedCase?.source && <a href={selectedCase.source.url} target="_blank" rel="noreferrer">연계 자료 · {selectedCase.source.type} · {selectedCase.source.title} ↗</a>}</div><FooterActions back={() => setStep(2)} next={generatePaths} nextLabel={loading ? "Gemini가 세 갈래 결을 만드는 중…" : "세 갈래 결 만들기 →"} disabled={loading} /></div>}
    {step === 4 && <div><Header eyebrow="세 길 — 파고드는 결" title="이 사례를 어디로 파고들래요?" description="Gemini가 학술 자료와 선택 사례를 바탕으로 만든 세 가지 구체적 탐구 결입니다. 실제 대상·조건·자료를 확인하고 한 길을 고르세요." /><div className="chosen-lens"><small>선택한 사례 · {lens.name}</small><strong>{selectedCase?.caseTitle || fallbackCase(topic, lens).caseTitle}</strong></div><div className="path-list">{activePaths.map((item, index) => <article key={item.title} role="button" tabIndex={0} onClick={() => setPathIndex(index)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setPathIndex(index); }} className={pathIndex === index ? "selected" : ""}><b>{index + 1}</b><div><small>구체 탐구 장면 · {item.focus}</small><h3>{item.title}</h3><strong>{item.question}</strong><p>{item.detail}</p>{item.source && <a href={item.source.url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>연계 자료 · {item.source.type} · {item.source.title} ↗</a>}</div></article>)}</div><FooterActions back={() => setStep(3)} next={generatePlan} nextLabel={loading ? "탐구 청사진을 만드는 중…" : "다음 →"} disabled={loading} /></div>}
    {step === 5 && <div><Header eyebrow="기본 탐구" title="이 길의 탐구를 한 번에" description="고른 길의 탐구가 어떻게 펼쳐지는지 한 번에 보여 드립니다." /><div className="blueprint"><article><small>무엇을 묻나</small><h2>{activePlan.basic.what}</h2></article><article><small>어떻게 알아보나</small><p>{activePlan.basic.how}</p></article><article><small>무엇을 만나게 되나</small><p>{activePlan.basic.discover}</p></article></div><FooterActions back={() => setStep(4)} next={() => setStep(6)} nextLabel="한 층 더 깊이 →" /></div>}
    {step === 6 && <div><Header eyebrow="더 깊이 — 1단" title="한 층 더 깊이" description="지금 관점 안에서 한 층 더 들어가는 세 갈래입니다." /><div className="deep-list">{activePlan.deepening.map((item, index) => <button key={item.title} onClick={() => setDeepIndex(index)} className={deepIndex === index ? "selected" : ""}><b>{index + 1}</b><div><h3>{item.title}</h3><strong>{item.text}</strong><p>{item.detail}</p></div></button>)}</div><FooterActions back={() => setStep(5)} next={() => setStep(7)} nextLabel="보고서 목차 만들기 →" /></div>}
    {step === 7 && <div><Header eyebrow="마무리 — 탐구 보고서 목차" title="탐구 보고서의 뼈대" description="선택한 주제와 관점, 탐구의 길, 심화 질문을 보고서의 구조로 묶었습니다." /><div className="report-title"><small>보고서 제목</small><h2>{reportTitle}</h2></div><Report title="I. 서론" items={[["1.1", "탐구 동기와 배경", `${topic}에 관심을 갖게 된 계기와 문제 상황을 구체적으로 정리합니다.`], ["1.2", "탐구 질문과 목적", activePlan.basic.what]]} /><Report title="II. 본론" items={[["2.1", "이론적 배경과 자료", `${topic}와 관련된 핵심 개념, 선행 자료, 비교 기준을 정리합니다.`], ["2.2", "탐구 방법", activePlan.basic.how], ["2.3", "심화 분석", `${deep.text}을(를) 중심으로 사례와 근거를 분석합니다.`]]} /><Report title="III. 결론" items={[["3.1", "발견과 해석", activePlan.basic.discover], ["3.2", "한계와 후속 질문", "이번 탐구의 한계와 다음에 더 확인하고 싶은 질문을 씁니다."]]} /><div className="save-actions"><button onClick={() => setStep(6)}>← 이전</button><button onClick={downloadReport}>텍스트로 저장</button><button className="next" onClick={() => window.print()}>인쇄 / PDF 저장</button></div></div>}
  </section></main>;
}
