...
export async function POST(request: Request) {
  ...
  const schema = `{\"perspectives\":[${keys.map((key) => `{\"key\":\"${key}\",\"question\":\"[관점명] 핵심 질문\",\"description\":\"2~3문장 설명\",\"example\":\"주제에 맞는 탐구 방향 예시\"}`).join(",")}],\"paths\":[{\"title\":\"...\",\"text\":\"...\",\"detail\":\"...\"},{\"title\":\"...\",\"text\":\"...\",\"detail\":\"...\"},{\"title\":\"...\",\"text\":\"...\",\"detail\":\"...\"}],\"basic\":{\"what\":\"...\",\"how\":\"...\",\"discover\":\"...\"},\"deepening\":[{\"title\":\"...\",\"text\":\"...\"},{\"title\":\"...\",\"text\":\"...\"},{\"title\":\"...\",\"text\":\"...\"}],\"report\":{\"title\":\"...\",\"intro\":\"...\",\"question\":\"...\",\"background\":\"...\",\"method\":\"...\"}}`;
  const prompt = `너는 학생의 막연한 아이디어를 다각도로 확장하는 사고 확장 전문가다. 주제는 "${topic}", 탐구 방향은 "${direction}"이다. 다음 10가지 사고 형식으로 주제를 분석하라: ${forms.join(", ")}. 각 관점은 반드시 [관점명] 핵심 질문 한 줄을 질문으로 제시하고, 학생이 구체적으로 탐구할 내용을 2~3문장으로 매력적으로 설명하며, 주제에 맞는 예시를 포함하라. 이후 선택 가능한 탐구 방향 3개, 청사진, 심화 질문 3개, 보고서 개요도 작성하라. 학생 수준에 맞게 쓰고, 결론을 단정하지 말며, 존재하지 않는 자료를 인용하지 마라. JSON만 반환하라. 형식: ${schema}`;
